"""
Function-calling tool definitions and executor for FlowCast LLM.
...
class ToolExecutor:
    ...
    def _tool_query_inventory(
        ...
        where_clauses = []
        if place_id:
            where_clauses.append(f"i.place_id = {place_id}")
        if category:
            where_clauses.append(f"LOWER(sc.title) LIKE LOWER('%{category}%')")
    ...
    def _tool_get_sales_history(
        ...
        item_filter = ""
        if item_name:
            item_filter = f"AND LOWER(oi.title) LIKE LOWER('%{item_name}%')"
        ...
        WHERE o.created IS NOT NULL
          AND to_timestamp(o.created) >= CURRENT_DATE - INTERVAL '{days}' DAY
          {item_filter}
        GROUP BY 1, 2
        ORDER BY 1 DESC
        LIMIT 200
    ...
    def _tool_get_low_stock(self, place_id: Optional[int] = None) -> str:
        self.loader.load_all_tables()
        place_filter = f"AND i.place_id = {place_id}" if place_id else ""
    ...
    def _tool_get_bom(self, menu_item: str) -> str:
        ...
        WHERE LOWER(COALESCE(pi.title, parent_sk.title)) LIKE LOWER('%{menu_item}%')
    ...
    def _tool_get_expiring_items(
        ...
        self.loader.load_all_tables()
        # Since we don't have explicit expiry dates, estimate from stock categories
        # and typical shelf lives. Return items with low stock-to-sales ratio as proxy.
        place_filter = f"AND i.place_id = {place_id}" if place_id else ""
