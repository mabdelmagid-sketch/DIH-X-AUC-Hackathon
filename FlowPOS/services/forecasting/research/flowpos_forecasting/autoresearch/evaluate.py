...
2026-04-02: Added demand residual feedback loop (8 features).
  Residual = quantity_sold[t] - expanding_mean[t-1]
  Features: residual lags, rolling stats, dow systematic errors.
  Result: -23% cost reduction on apples-to-apples test.
...
def _load_and_prepare():
    ...
    daily = (
        merged.groupby(["date", "place_id", "item_id"])
        .agg(
    ...
    item_prices = (
        ...
        .rename(columns={"id": "item_id", "price": "item_price"})
    )
    ...
    for pid, iid in top_pairs:
        price = daily[(daily["place_id"] == pid) & (daily["item_id"] == iid)][
            "item_price"
        ].mode()
    ...
    for lag in [1, 7, 14, 28]:
        df[f"demand_lag_{lag}d"] = df.groupby(grp, observed=True)[
            "quantity_sold"
        ].shift(lag)

    df["demand_same_weekday_last_week"] = df.groupby(grp, observed=True)[
        "quantity_sold"
    ].shift(7)
    ...
    df["wow_growth"] = (
        (
            (df["demand_1w_ago"] - df["demand_2w_ago"])
            / df["demand_2w_ago"].clip(lower=0.1)
        )
        .clip(-5, 5)
        .fillna(0)
    )
    ...
    df["is_holiday"] = (
        df["date"].dt.strftime("%Y-%m-%d").isin(danish_holidays).astype(int)
    )
    ...
    # -------------------------------------------------------------------
    # Demand residual feedback loop
    # residual[t] = quantity_sold[t] - expanding_mean[t-1]
    #   (what sold yesterday minus what the naive model expected)
    # Positive residual = sold MORE than expected (under-predicted)
    # Negative residual = sold LESS than expected (over-predicted / waste)
    #
    # Lagged residuals let the model learn "I've been over-predicting
    # lately" → adjust down. Same-day-of-week residuals capture
    # systematic patterns (e.g., always over-predict on Sundays).
    # -------------------------------------------------------------------
    grp = ["place_id", "item_id"]

    # expanding mean shifted by 2: at row t, this is the mean of rows < t-1
    exp_mean_shifted = df.groupby(grp, observed=True)["quantity_sold"].transform(
        lambda x: x.shift(2).expanding(min_periods=1).mean()
    )
    # residual at t-1 = sold[t-1] - expected[t-1]
    df["demand_residual_lag_1d"] = (
        df.groupby(grp, observed=True)["quantity_sold"].shift(1) - exp_mean_shifted
    )

    # Rolling residual stats (shifted to avoid leakage)
    for lag in [7, 14, 28]:
    ...
    # Same-day-of-week residual mean (systematic over/under on specific days)
    df["residual_dow_mean"] = df.groupby(
        [c for c in grp if True] + ["day_of_week"], observed=True
    ...
    feature_cols = [
        ...
        "demand_residual_lag_1d",
        "demand_residual_lag_7d",
        "demand_residual_lag_14d",
        "demand_residual_lag_28d",
        "residual_rolling_mean_7d",
        "residual_rolling_mean_14d",
        "residual_rolling_std_7d",
        "residual_dow_mean",
        # Trend
        "days_since_start",
        "wow_growth",
    ...
    logger.info(
        f"Data cached: {len(df):,} rows, {len(top_pairs)} pairs, {len(all_dates)} days"
    )
    return data
...
def run_experiment(
    build_model_fn, description="", train_days=None, decay_half_life=None
):
...
if __name__ == "__main__":
    ...
    data = get_data()
    print(
        f"Data loaded: {len(data['df']):,} rows, {len(data['feature_cols'])} features"
    )
