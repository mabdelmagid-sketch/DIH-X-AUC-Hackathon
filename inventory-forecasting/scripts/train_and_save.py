"""Offline training script: builds features, trains both models, saves predictions to disk.

Run this ONCE before launching the dashboard:
    cd inventory-forecasting
    python scripts/train_and_save.py

The dashboard will then load the saved predictions instantly instead of
...
def main():
    ...
    # ── 2. Build features ──────────────────────────────────────────────────
    logger.info("Step 2/5: Building features (top 20 items per store)...")
    ...
    logger.info("Step 3/5: Training Balanced model (30% XGB + 70% MA7)...")
    ...
    logger.info("Step 4/5: Training Waste-Optimized model (85% of balanced)...")
    ...
    # ── 5. Save to disk ───────────────────────────────────────────────────
    logger.info(f"Step 5/5: Saving to {PREDICTIONS_PATH}...")
